...
```java
    ...
    注意：可序列化类中所有属性要是可序列化的(基本数据类型和String默认是可序列化的)
          static/transient修饰的属性不能序列化，也就是说序列化的时候明明给值了比如String我给了值但是在反序列化的时候没值是null
          
          不过一般序列化传值很少穿Person这种，一般都是json(字符串格式，默认是可序列化的)
...
模拟出错：两个线程，A 要扩容的时候阻塞了，等 B 扩完后。A 醒了要扩的那个状态还保留在没扩之前的状态

1）两个线程，两个指向：

<img src="http://image.zzq8.cn/img/202211281758986.png" alt="image-20221128175817858" style="zoom:67%;" />



2）线程 1 扩容完，线程 2 醒来

<img src="http://image.zzq8.cn/img/202211281759166.png" alt="image-20221128175930037" style="zoom: 67%;" />



3）B.e 醒来后却还指向 3，两个线程先后顺序已经颠倒，**因为头插法打乱了链表的顺序**



<img src="http://image.zzq8.cn/img/202211281751659.png" alt="image-20221128175156295" style="zoom:67%;" />





#### 一、jdk1.7 HashMap头插法多线程环境下链表成环的场景怎么形成
...
```java
		...
		//3.void fill(int[] a,int val):将指定值填充到数组之中。      例如下面使数组中都是10
		Arrays.fill(arr1,10);
		System.out.println(Arrays.toString(arr1));
