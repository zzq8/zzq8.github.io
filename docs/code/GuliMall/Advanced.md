...
> 幂等性实现方案（三种）：
>
> * 通过唯一请求标识符（Idempotency Key）和内存中已有的匹配【Redis】
>   * token 也是这种
>     * 执行业务前存 token
>     * 真正执行时候的 if 判断是否还在（存在表示第一次请求）**删 token** 继续执行业务
> * 通过数据库唯一约束
> * **锁机制：确保在同一时间内只有一个请求在处理，从而保证幂等性**
>   * 数据库层面的锁（乐观、悲观）
>   * 分布式锁
>
> 
...
>
> 针对分布式事务，又诞生了一些新的理论。    ==分布式事务基础理论 ---> CAP 理论和 BASE 理论==
...
* `2PC`（Seata AT事务模式是这个的一个变形）还有3PC  【2 phase commit 二阶提交】  准备阶段+提交阶段
  注意和MySQL写日志的两阶段提交区分，是不一样的东西。Seata AT是第一阶段提交+2第二阶段看要不要补偿-2  只适合一般的分布式事务不合适高并发     虽然它有补偿机制，但他就是刚性事务



* 柔性事务-`TCC` 事务补偿型方案：相当于3PC的手动版 【TCC 是 Try、Confirm、Cancel 三个词的缩写，它分为三个阶段】
