...
1->2 crash: 事务失效, 那么redolog里还有无效数据吗   (那不是binlog少一处吗)
...
下图中每条记录的类型用数字0123表示了，

- 0就是真实的数据记录
- 1就是只有索引+页号的记录，2是Inf记录，3是Sup记录

树的每个节点是一个页，其中叶子节点是真正存储数据的，又叫`数据页`，而其他节点只存储索引值+页号，叫做`目录页`



非聚簇索引 (2, 20就是数据是2 id是20) 
...
>
> [使用！ ](https://developer.aliyun.com/ask/281197) mysql查询**==优化器==**会判断纠正这条sql语句该以什么样的顺序执行效率最高，最后才生成真正的执行计划
