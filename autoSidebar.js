...
walkSync(curPath, function (filePath, stat) {
	if (".md" == path.extname(filePath).toLowerCase()//后缀是.md
		...
		var relativeFilePathArr = relativeFilePath.split(require('path').sep); //这里可以看情况改   .split('/')       XD:Win & Mac Not Same Here!!!
...
fs.writeFile(curPath + require('path').sep +'_sidebar.md', sidebarTxt, function (err) {
	if (err) {
		console.error(err);
	}
});
